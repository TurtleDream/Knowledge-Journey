...
app.post('/api/llm', async (req, res) => {
  ...
  try {
    ...
    if (cfg.provider === 'yandex') {
      // URL ровно как во фронтенд-клиенте (generateYandex): foundationModels в
      // camelCase и /completion. foundation-models/v1/... отдаёт 404 — проверено.
      url = cfg.apiUrl || 'https://llm.api.cloud.yandex.net/foundationModels/v1/completion';
      headers = { Authorization: `Api-Key ${cfg.apiKey}` };
      // model можно задать как "folderId/yandexgpt-lite" (как в настройках фронта),
      // либо folderId отдельно + имя модели
      let folderId = cfg.folderId ?? '';
      let mdl = cfg.model || 'yandexgpt-lite';
      if (mdl.includes('/')) {
        [folderId, mdl] = mdl.split('/');
      }
      ...
      if (!r.ok) {
        res.status(r.status).json({ error: `${cfg.provider} ошибка ${r.status}: ${await r.text()}` });
        return;
      }
      const data = await r.json();
      const text = data?.result?.alternatives?.[0]?.message?.text;
      if (!text) {
        res.status(502).json({ error: 'пустой ответ YandexGPT' });
        return;
      }
      res.json({ text });
      return;
